...
hosts() {
    ...
    then
        ...
        if [ "$COMPUTER_NAME" == "Aaron's MacBook Pro" ]; then 
            ...
            sudo bash -c "echo 127.0.0.1 casapps.test classnav.test esurvey.test eval.test iadvise.test icoach.test coursereflection.test ses.test tps.test provost-apps.test >> /etc/hosts"
        fi
    fi
}
